...
- **Model-seat transparency & safety-routing**: the desktop/web Claude seat can change model mid-project (it's been reported that **Claude Fable 5 may route some sensitive biology / chemistry / cybersecurity requests to an Opus model** — e.g. Verge coverage; recorded as *reported, not self-verified*). House rule: **lead every relay with the current model seat** (e.g. "Current model seat: Fab5"); if the model appears to fall back or change, **say so before proceeding**. Crucially: **preserve accurate scientific terminology and split sensitive vs. ordinary engineering work honestly — do NOT deliberately disguise biology/chemistry/cyber/etc. wording to evade a classifier.** If safety handling changes the available model or task capability, **stop and report rather than game the system.** (This is benign for Medusa anyway: it genuinely *is* topology + distributed computation; the bio/chem items in the theory ledger are labelled metaphors, so describing it accurately is just accuracy, not evasion.)
