...
- **CI gate (FULL SUITE — #165 COMPLETE & CLOSED)**: `verify-python` runs the **entire maintained suite** via `python -m pytest tests/ -q` on every PR/push (it began as just the 2 nextness files under #167). Current state ≈ **718 passed, 33 skipped, 0 failed, 0 collection errors** (see the latest `verify-python` run for live counts — numbers grow as skip-conversion proceeds). The coverage-broadening arc (#172→#186, Tiers 0–4D) is merged and **[Issue #165](https://github.com/Goldislops/UtilityFog-Fractal-TreeOpen/issues/165) is closed as completed**. **Skip-conversion** since then has added pip deps to CI so more suites RUN instead of skip: `matplotlib`/`flask`/`pyzmq` (#190) and `openai` (this PR). CI install: `numpy pytest pytest-asyncio matplotlib flask pyzmq openai`. Remaining skips are **deliberate**: the unbuilt `uft_ca` Rust extension (~25, → #180) + a few internal sub-skips. (Run tests locally as hygiene; the old "CI runs nothing" blind spot is long fixed.)
