...
5. **Killable at any time.** SIGTERM / Ctrl+C must leave a consistent log state and have zero engine impact. No partial writes, no orphaned subscriptions. Concretely: the canonical `nextness_runs.jsonl` is published transactionally — each record is staged to a same-directory temporary file (previous log bytes + exactly one new newline-terminated record), flushed and fsynced, then atomically swapped over the canonical path with `os.replace`. A failure or kill at any stage leaves the canonical log either as the complete previous version or the complete new version; a hard kill can at worst leave a stray `nextness_runs.jsonl.*.tmp` staging file, which is never read as log data (ordinary handled failures remove it). Write authority and permissions: two preconditions are necessary — write access to the existing log itself (a non-writable log is refused with `PermissionError` before any staging exists), **and** directory permission to create, replace and clean a same-directory staging entry, which is an unavoidable precondition of same-directory atomic replacement rather than parity with every historical append-permission layout. In a layout where the process can write a pre-created log but cannot create directory entries, direct append would have succeeded; transactional publication deliberately fails closed there (`PermissionError`, canonical log untouched) instead of silently falling back to a non-atomic append. An existing log keeps its permission bits across publication, and a first-time log is created with the ordinary `0o666 & ~umask` mode — never the staging file's restrictive `mkstemp` mode. No ownership/ACL or platform-specific access-control equivalence is claimed beyond what the tests exercise. The exception a caller observes is always the primary failure; a secondary cleanup-close failure never masks it (a close failure with no earlier failure still propagates). Scope honestly stated: this guarantees in-process/kill consistency for a single writer; only permission bits are preserved (on Windows just the read-only flag) — **not** ownership, ACLs, or extended attributes; `os.replace` changes the file's identity (inode), so hard links and already-open readers keep the old file; and there is **no** power-loss durability claim (the directory entry is not fsynced) and no multi-writer safety.
