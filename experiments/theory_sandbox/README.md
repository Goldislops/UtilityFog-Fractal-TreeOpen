...
> **Status**: policy **and live inventory**. **Six toy scripts now exist across five toys** (Toy #1 Galton/Fourier diffusion; Toy #2 stochastic-escape/annealing; Toy #3 decaying scent-trail tracking; Toy #4 Janus gradient; Toy #5 passive "MOF" trap — **v0 latch-capture + v1 Strict-Passive release-trap**) — each **non-canonical**, each landed as its own reviewed PR. **Toy #6 (Discrete Compliant "yield-then-lock" trap): lock framing analytically closed** — no script (see §6b + `TOY_06_COMPLIANT_TRAP_ADDENDUM_2.md`). **The trap arc is now closed: the passive local rule-mask trap paradigm is exhausted for identity-preserving trapping of a canonical Conway glider** across its full conceptual perimeter — over-bind / block / lock / confine **and the trap-adjacent encode-and-release / state-transduction** door (capstone §2b) — see [`TRAP_ARC_CAPSTONE.md`](TRAP_ARC_CAPSTONE.md). The remaining out-of-class arcs (placed-object reflector, topology/wrap, active shutter, reversible-CA store-and-reconstruct) and Janus+MOF coupling are all **parked + unauthorized**. Each new toy still lands only as its own small PR after its inception note is reviewed (one toy per PR; see §4 + §6).
>
...
*Toy #5 trap-arc **synthesis**: [`TOY_05_SYNTHESIS.md`](TOY_05_SYNTHESIS.md) — what v0 (capture-with-transformation) and v1 (inert-wall shear) established, the capture↔identity tension, the explicit non-claims, and the gated forward queue (compliant "yield-then-lock" trap / shutter / Janus+MOF coupling).*

*Whole-arc **capstone**: [`TRAP_ARC_CAPSTONE.md`](TRAP_ARC_CAPSTONE.md) — the durable close of the trap arc. Records that the **passive local rule-mask trap paradigm is exhausted** for identity-preserving trapping of a canonical Conway glider across **all four framings** — over-bind/latch (Toy #5 v0, capture-with-transformation), block/freeze (Toy #5 v1, inert-wall shear), lock/compliance (Toy #6, analytically collapses — glider identity is motion), confine/reflection (read-only feasibility, collapses or leaks into placed-object/topology/active-control primitives). Bounded to this target + framing; **not** a universal CA impossibility theorem. Parks the four out-of-class arcs + Janus+MOF coupling, all unauthorized.*
...
| Confine-not-lock trap | Ledger entry 15 (Toy #6 Addendum 2) | **Feasibility assessed read-only → collapses or leaks out of class** (see [`TRAP_ARC_CAPSTONE.md`](TRAP_ARC_CAPSTONE.md) §2a): a uniform local rule-mask cannot reflect/bound a glider while preserving identity — candidates reduce to shear/annihilation/transform/pass-through, or leak into placed-object/topology/active-shutter primitives. The **rule-mask** confine is closed; a **topology/wrap** confine is a *different primitive*, parked + unauthorized. |
| Encode-and-release / state-transduction trap ("stopped-light") | Ledger entry 15 (Toy #6 Addendum 2) | **Feasibility assessed read-only → collapses or leaks out of class** (see [`TRAP_ARC_CAPSTONE.md`](TRAP_ARC_CAPSTONE.md) §2b): a passive-local no-clock rule-mask cannot encode a glider and faithfully re-emit it — fails via decode-timing/shutter-leakage, Conway non-reversibility/information-loss, and placed-object/catalyst leakage; degenerate no-decode case = Toy #5 v0 latch relabeled. The **rule-mask** encode-release is closed; **reversible-CA** store-and-reconstruct and **placed-object** memory cells are *different primitives*, parked + unauthorized. |
