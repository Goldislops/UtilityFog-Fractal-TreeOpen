...
- each arm maintains **its own** next-unvisited waypoint index into this shared ordering, plus a per-arm **`reached`** set of every cell it has stood on (initialised to `{start}`, and updated after *every* tracker move — strict-ascent moves included);
- **Pre-advance before moving (erratum fixes — Codex P2 r3441709727 + #247 r…aad):** *before* each fallback movement, skip any waypoint this arm has **already reached** — `while idx < len(waypoints) and waypoints[idx] in reached: idx += 1` (the `idx < len(waypoints)` bound prevents an out-of-range read once every waypoint has been reached; on a 1024-cell grid with a 192-move budget the list cannot actually exhaust, so the bound is defensive). Skipping on **`waypoints[idx] in reached`** (not merely `tracker_pos == waypoints[idx]`) is required because a single cardinal step toward a *diagonal* waypoint can **transit a later waypoint cell** before it becomes the current target (e.g. heading `(16,16)→(15,15)` steps onto `(15,16)` first); skipping only the current cell would let the sweep walk *back* to that already-visited cell. The reached-set form keeps it a true next-**unvisited** expanding-ring sweep, discards the distance-0 start cell on the first tick, and means the fallback never stalls, emits a zero-length "move", or re-targets a visited cell;
...
- the fallback **never targets an already-reached waypoint** (`waypoints[idx] not in reached` after pre-advance) — so the sweep stays a true next-unvisited expanding ring;
