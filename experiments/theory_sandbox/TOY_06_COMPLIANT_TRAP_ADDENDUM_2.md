...
The flexible-polyhedra analogy (Ledger Entry 15) supplied the *"yield while preserving an invariant, then hold"* intuition — a flexible polyhedron deforms while the **Bellows** result keeps its enclosed **volume** invariant. **But the invariant relevant here is NOT literal volume** (there is no physical volume, no elasticity, no Hooke's law, no material). The only admissible invariants are **discrete identity invariants**: live-cell count, bounding box, phase/orientation, period signature, post-release re-emergence, or an **encoded identity state** (for the encode-and-release hypothesis). Moreover, the analogy *breaks precisely at "lock"*: a polyhedron **has a held shape** to preserve while flexing; **a glider does not** — which is the whole reason the lock framing collapses. The polyhedra gave vocabulary and a failure-mode structure, never physics.
