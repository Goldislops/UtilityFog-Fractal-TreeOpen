...
> **Entries 11–13 — four-topic intake (relayed 2026-06-11 by AURA/Kev; cross-checked by Jack against primary/reputable sources).** Three physics/materials metaphors. The relay's *fourth* item (a model-seat / model-fallback note) is **not** a physics entry — it lives in `AGENT_HANDOFF.md` → Operational Conventions, reframed away from any classifier-evasion framing. As always: labelled theory, not law.

### 11. Passive alignment under a global field (pigeon-liver magnetoreception)
- **Source**: AURA/Kev video intake **plus a reported *Science* study**; DOI `10.1126/science.ady2486` and the exact title/"superparamagnetic" wording are **supplied through the intake — direct publisher-link confirmation pending**. The *finding* is corroborated by reputable reporting (*The Times*; Max-Planck-Institute-of-Animal-Behaviour / University-of-Bonn involvement) and has **public skeptics** (*Scientific American*).
...
- **Guardrails/caveats**: Do **not** call it a "proven quantum compass" — the reported mechanism is **superparamagnetic (classical)**, not quantum (and "superparamagnetic" is *source-supplied exact wording, not independently confirmed* here). Do **not** claim "zero compute" or zero biological energy cost. Do **not** claim the transduction mechanism is settled (it is contested). Do **not** introduce "superparamagnetic nodes" into architecture.
- **Linked**: entry 1 (emergence), entry 2 (Swarm Hunter guardrail).

### 12. Event-driven state change (weak-interaction metaphor)
- **Source**: AURA/Kev video intake — established particle physics. Durable reference: **Particle Data Group, "CKM Quark-Mixing Matrix," 2024 Review of Particle Physics** (charged-current W± interactions couple physical up-/down-type quarks via the CKM matrix).
...
- **Claim (corrected)**: Keep three geometry objects separate: **Jessen's orthogonal icosahedron** is rigid but not infinitesimally rigid ("shaky"), not a genuinely flexible polyhedron; **Connelly's flexible polyhedron / Connelly sphere** is Robert Connelly's 1977 first non-self-intersecting embedded flexible polyhedron in 3D, with **18 triangular faces**; **Steffen's polyhedron** is a later simpler embedded flexible polyhedron with **14 triangular faces**. The Bellows theorem applies to true flexible polyhedra (volume invariant under flexing), not casually to Jessen's shaky object.
